...
/// ----- Gestion affichage/style card listen à faire
  if (listenToDo) {
    styleListenToDo = (
      <Link to="/listen" >
        <Badge count={1}>
          <Card hoverable className='card' cover={<img alt="Faire mon listen" style={{height:'215px'}} src={opinions}/>}>
    ...
    styleListenToDo =(
      <Card className='card-disabled' cover={<img style={{height:'215px'}} alt="Faire mon listen" src={opinions}/>}>
...
/// ----- Gestion affichage/style card listen à voir
  if (listenToSee) {
    styleListenToSee = (
      <Card hoverable onClick={async() => {await getSeeListen(props.user._id);showModal()}} className='card'cover={<img style={{height:'215px'}} alt="Voir mon listen" src={reading}/>}>
        <Card.Meta title="Voir mon listen"  />
      </Card> 
    )
  } else {
    styleListenToSee = (
      <Card className='card-disabled'cover={<img style={{height:'215px'}} alt="Voir mon listen" src={reading}/>}>
          ...
          <Row gutter={[20, 24]} justify='center' align= 'middle' style={{height:'75%', width:'100%'}}>
            ...
            <Col span={6}>
              <Link to="/historique-collab" >
                <Card hoverable className='card' cover={<img alt="Mon historique" style={{height:'215px'}} src={searching}/>}>
